...
var draw = (0, _svg2.default)(document.body).addClass('canvas');
var overlay = draw.text('').move(10, 10).font({
  family: 'monospace',
  size: 12
}).style({
  'user-select': 'none'
...
function generateRDataFrame() {
  ...
  var ys = 'c(';
  var groups = 'factor(c(';
  Object.keys(pointArray).forEach(function (i) {
    if (i > 0) {
      xs += ',';
      ys += ',';
      groups += ',';
...
draw.on('click', function (e) {
  var newPointSVG = draw.circle(10).fill(colorPalette[currentColor - 1]).move(e.offsetX, e.offsetY).attr('color-group', currentColor);
...
function showText(textGenerator) {
  var csvContent = 'data:text/text;charset=utf-8,' + textGenerator();
